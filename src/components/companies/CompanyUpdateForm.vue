        ...
        <v-stepper-items>
          ...
          <v-row>
            ...
            <v-col class="text-md-right text-sm-center" cols="6" md="6" sm="12">
              ...
              <v-btn class="ma-2" color="primary" :disabled="!isCurrentStepValid" @click="nextStep">
                {{ nextButtonName }}
              </v-btn>
            </v-col>
          </v-row>
    ...
    <v-dialog v-model="showConfirmDialog" max-width="290">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="isUpdating" text @click="runUpdate"> Continuar </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
@Component({})
export default class CompanyUpdateForm extends Vue {
  @Prop({ type: Object, default: homeRoute })
  cancelLocation: RawLocation;
  ...
  cancel() {
    if (this.cancelLocation) {
      this.$router.push(this.cancelLocation);
    }
  }
